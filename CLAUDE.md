...
- [ ] Power BI semantic model on MARTS (SSAS replacement story)
- [ ] One paginated report (SSRS replacement story)
- [ ] Tag `v0.3.0-serve`
...
- **Which CORE domain to model as Data Vault 2.0?** Probably transactions (most history complexity), but TBD — confirm before v0.2.0 build.
...
- **Power BI vs Streamlit for serving** — Streamlit is live. Power BI still planned for the "replaces SSAS / SSRS" legacy-parity demo (v0.3.0).
...
3. **Model the warehouse — `v0.2.0-model-the-warehouse` (built; awaiting tag)**
   Star Schema (6 dims + 4 facts + 3 MARTS), Type 2 `dim_policy_history` via dbt snapshot, and full Data Vault 2.0 transactions domain (4 hubs + 3 links + 2 sats with hashdiff guards) all live in CORE. ADR-0015. `dbt build` PASS=72 WARN=0 ERROR=0.

4. **Serve — `v0.3.0-serve`**
   Streamlit in Snowflake live with domain dashboards. Outstanding: Power BI semantic model on MARTS + one paginated report (SSAS / SSRS legacy-parity demo).
